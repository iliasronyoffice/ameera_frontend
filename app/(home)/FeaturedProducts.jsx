...
// Skeleton component (shows immediately)
  ...
  return (
    <div className="featured-product-section container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-4">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="bg-gray-100 animate-pulse rounded-lg" style={{ height: '280px' }}></div>
        ))}
      </div>
    </div>
  );
...
// Optimized memory cache (faster than localStorage)
const memoryCache = {
  data: null,
  timestamp: null,
...
export default function FeaturedProducts() {
  ...
  // Initialize progress bar with better performance
  useEffect(() => {
    ...
    const timer = setTimeout(initProgressBar, 100);
    ...
    return (
      <div className="featured-product-section container mx-auto px-4 py-8">
        <div className="p-4 bg-red-50 border border-red-200 rounded-lg text-center">
          <p className="text-red-700 mb-2">Failed to load featured products</p>
          <button 
            onClick={() => {
              setLoading(true);
              setError(null);
            ...
            className="px-4 py-2 bg-main text-white rounded transition"
          >
            Try Again
          </button>
        </div>
  ...
  return (
    <div className="featured-product-section container mx-auto px-4 py-8">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        >
          <span className="bg-white p-1 md:p-2 rounded-md">
            <svg width="10" height="10" viewBox="0 0 15 15" fill="none" className="w-3 h-3 md:w-4 md:h-4">
              <path
      ...
      {/* Swiper - Shows immediately with products */}
      <Swiper
